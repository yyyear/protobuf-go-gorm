...
var (
	...
	datadriver           goImportPath = protogen.GoImportPath("database/sql/driver")
	encodingJson         goImportPath = protogen.GoImportPath("encoding/json")
	gorm                 goImportPath = protogen.GoImportPath("gorm.io/gorm")
)
...
func genMessage(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo) {
	...
	for i, field := range m.Fields {
		...
		if i == 0 {
			if strings.Contains(string(field.Comments.Leading), "Valuer") || strings.Contains(string(field.Comments.Leading), "valuer") {
				g.P("// Value 实现`Valuer`接口")
				g.P("func (x ", m.GoIdent, ") Value() (", datadriver.Ident("Value"), ", error) {")
				g.P("return ", encodingJson.Ident("Marshal"), "(&x)")
				g.P("}")
				g.P()
				g.P("// Scan 实现`Scanner`接口")
				g.P("func (x *", m.GoIdent, ") Scan(val interface{}) error {")
				g.P("s := val.([]byte)")
				g.P("return ", encodingJson.Ident("Unmarshal"), "(s, x)")
				g.P("}")
				g.P()
				break
			}
	...
	varName := m.GoIdent.GoName + "Valuers"
	g.P("type ", varName, " []*", m.GoIdent)
	
	g.P("func (x ", varName, ") Value() (", datadriver.Ident("Value"), ", error) {")
	g.P("return ", encodingJson.Ident("Marshal"), "(&x)")
	g.P("}")
	g.P()
	g.P("// Scan 实现`Scanner`接口")
	g.P("func (x *", varName, ") Scan(val interface{}) error {")
	g.P("s := val.([]byte)")
	g.P("return ", encodingJson.Ident("Unmarshal"), "(s, x)")
	g.P("}")
	g.P()
...
func genMessageInternalFields(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo, sf *structFields) {
	g.P(genid.State_goname, " ", protoimplPackage.Ident("MessageState `gorm:\"-\"`"))
...
func genMessageField(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo, field *protogen.Field, sf *structFields) {
	...
	gormValue := fieldGORMTagValue(field)
	tags := structTags{}
	if gormValue == "" {
		tags = structTags{
...
func genMessageGetterMethods(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo) {
	for _, field := range m.Fields {
		if field.Desc.Name() == "gormvaluer" || field.Desc.Name() == "gormvaluer" {
			continue
		}
...
func fieldGoType(g *protogen.GeneratedFile, f *fileInfo, field *protogen.Field) (goType string, pointer bool) {
	...
	case field.Desc.IsList():
		if strings.Contains(string(field.Comments.Leading), "Valuer") || strings.Contains(string(field.Comments.Leading), "valuer") {
			return g.QualifiedGoIdent(field.Message.GoIdent) + "Valuers", false //
		}
		// varName := m.GoIdent.GoName + "Valuers"
		//return g.QualifiedGoIdent(field.Message.GoIdent) + "Valuers", false //
		return "[]" + goType, false
...
func fieldJSONTagValue(field *protogen.Field) string {
	...
	tempData := reg.FindAllString(field.Comments.Leading.String(), -1)
	for _, v := range tempData {
		if v == "json<->" {
			return "-"
		}
		return v[5:len(v)-1] + ",omitempty"
	}
	return string(field.Desc.Name()) + ",omitempty"
}

func fieldGORMTagValue(field *protogen.Field) string {
	if field.GoName == "id" || field.GoName == "ID" || field.GoName == "Id" || field.GoName == "iD" {
		return "primaryKey;index"
	}
	reg := regexp.MustCompile("gorm<(.*?)>")
	tempData := reg.FindAllString(field.Comments.Leading.String(), -1)
	for _, v := range tempData {
		return v[5 : len(v)-1]
	}
	return "" //, field.Desc.Options().(*descriptorpb.FieldOptions).GetDeprecated()
}
